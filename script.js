document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyFiltersAndSort() {
    ...
    if (selectedSubject !== "all") {
      filteredCourses = filteredCourses.filter(
        (course) => course.category === selectedSubject
      );
    }
    // Filter by Year
    if (selectedYear !== "all") {
      filteredCourses = filteredCourses.filter(
        (course) => course.year.toString() === selectedYear
      );
    }
    // Filter by Level
    if (selectedLevel !== "all") {
      filteredCourses = filteredCourses.filter(
        (course) =>
          Math.floor(parseInt(course.id) / 100) * 100 ===
  ...
  function renderSnippets(snippets) {
    ...
    const html = snippets
      .map(
        (item) => `
        <a href="/courses/course.html?id=${item.courseId}" class="snippet-link">
      ...
      const cardHTML = `
                <div class="course-card" data-category="${course.category}">
                    <a href="/courses/course.html?id=${course.id}">
                        <img src="${course.image}" alt="Image for ${
        course.title
      }">
                    </a>
                    <a href="courses/course.html?id=${course.id}">
